...
@section('content')
    <h4 class="text-center pb-3">{{ $astroname }} на
        <b>
            <span
                class="attention cntdate">{{ isset($fordate) ? date("d-m-Y", strtotime("$fordate")) : date('d.m.Y', time()) }}</span>
{{--
            для
            <span
                class="attention cntdate">{{ isset($mydate) ? date("d-m-Y", strtotime("$mydate")) : date('d.m.Y', time()) }}</span>
--}}
