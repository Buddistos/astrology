...
function deleteResizedImage($imageFile)
{
	$imageResizers = [
		'maximum',
		'large',
    ...
    $withoutWatermarkFile = $path . '/' . str_replace('.files/' . $_SERVER['HTTP_HOST'], '.files/' . $_SERVER['HTTP_HOST'] . '/original', str_replace('../', '', $imageFile));;
    if (file_exists($withoutWatermarkFile)) {
        unlink($withoutWatermarkFile);
    }

	foreach ($imageResizers as $resize) {
		$filepath = $path . '/' . str_replace('.files/' . $_SERVER['HTTP_HOST'], '.files/' . $_SERVER['HTTP_HOST'] . '/resized/' . $resize, str_replace('../', '', $imageFile));

		if (file_exists($filepath)) {
			unlink($filepath);
			$fileName = end(explode('/', $filepath));
			$dirPath = preg_replace('~' . $fileName . '~', '', $filepath);
			if(count(scandir($dirPath)) <= 2) rmdir($dirPath);
		}
