	...
	class Request {
		...
		protected function _check_file_error($error) {
			switch ($error) {
				...
				case 2:
					...
					$this->_file_error = 'Загружаемый файл превышает допустимый максимальный размер (' . $max_file_size . ')';
					break;
				case 3:
					$this->_file_error = 'Загружаемый файл был загружен лишь частично';
					break;
				case 4:
					$this->_file_error = 'Файл не был загружен';
					break;
				case 6:
					$this->_file_error = 'Отсутствует временная папка';
					break;
				case 7:
					$this->_file_error = 'Не удалось записать файл на диск';
					break;
				case 8:
				default:
					$this->_file_error = 'Ошибка загрузки файла';
					break;
			}
		}
		...
		protected function _upload_file($path, $arr, $mode) {
			...
			if ($this->_validation($file_name, 'string')) {
				...
				echo json_encode(array('error' => 'недопустимое имя файла'));
				exit();
			}
			echo json_encode(array('error' => 'это расширение не поддерживается'));
			exit();
		}
...
EOT;
		...
		public function Geticonsgallery() {
			...
			echo json_encode(array('error' => 'Иконки не существуют'));
			exit();
		}
...
//								var_dump('video/gallery/' . $video . $type);die;
		...
		public function Download() {
			...
			//$this->installProjectToSite($file_name);
			exit();
		}
		...
		public function installProjectToSite($file_name)
		{
			//Развертывание  созданных страниц в папку сайта
		}
	}
