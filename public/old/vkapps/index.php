...
$allgoro = mysqli_query($mysql, $check);
...
if ($mid) {
    ...
    if ($exists) {
        ...
        $birthday = mysqli_result($result, 0, 'vkuserbd');
    } else {
        die ("Извините, непредвиденная ошибка (#1). Пожалуйста, обратитесь к разработчикам приложения.");
    }
}
if ($fid) {
    ...
    if ($exists) {
        ...
        die ("Извините, непредвиденная ошибка (#2). Пожалуйста, обратитесь к разработчикам приложения.");
    }
}
...
if ($gsk && $uid) {
    ...
    $result = mysqli_query($mysql, $query);
    $exists = mysqli_num_rows($result);
    $viewcost = $id_gorogroup == 5 ? 7 : 2;
    if (!$exists && $stars - $viewcost > 0) {
        $stars -= $viewcost;
        $query = "INSERT INTO _vkviews (vkgsk, vkuser, vkfriend) VALUES ('$gsk', '$mid', '$gid');";
        $result1 = mysqli_query($mysql, $query);
        if (!$result1) {
            die ("Извините, непредвиденная ошибка. Пожалуйста, обратитесь к разработчикам приложения." . mysqli_error());
        }
...
<head>
    ...
    <link rel="stylesheet" href="//ajax.googleapis.com/ajax/libs/jqueryui/1.10.4/themes/smoothness/jquery-ui.css"
          type="text/css"/>
    <link rel='stylesheet' href='//fonts.googleapis.com/css?family=Seymour+Display|Marmelad&subset=cyrillic'
          type='text/css'>
        ...
        $(function () {
            ...
            VK.init(function () {
                ...
                VK.api("users.get", {user_ids: uids, fields: "bdate, sex, photo_medium"}, function (data) {
                    ...
                    bdate = bdate && bdate.split('.').length == 3 ? bdate : "<?php echo $birthday; ?>";
                    if (bdate == undefined) {
                        $.fancybox.open({
                            content: "<h3>Внимание!</h3><p class='aligncenter'>День рождения не определен.<br>Показан прогноз на случайную дату.<br>Пожалуйста, измените день рождения в <a href='//vk.com/edit' target='_blank'>настройках.</p>",
                            topRatio: 0,
                            margin: [100, 20, 20, 20],
                            autoHeight: true,
                            height: 'auto'
                        ...
                        if (bdate && bdate != "0000-00-00") {
                            ...
                            $.fancybox.open({
                                content: "<h3>Внимание!</h3><p class='aligncenter'>День рождения пользователя не определен.<br>Гороскоп будет не точным.<br>Пожалуйста, не тратьте свои звезды напрасно.</p>",
                                topRatio: 0,
                                margin: [100, 20, 20, 20],
                                autoHeight: true,
                                height: 'auto'
...
<div class="container">
    ...
    <div class="block">
        <?php if ($gsk): ?>
            <div id="goback"><a href='<?php echo($fid ? "?fid=$gid" : "/vkapps/"); ?>'><img src="/images/arrow-left.png"
                                                                                            width="40"/></a></div>
        <?php elseif ($fid): ?>
        ...
        <?php else: ?>
            <div style="margin-left: 295px; margin-top: -5px; width:180px; height: 70px; position: absolute;"><a
                    id="minigraph" href="javascript:void(0);" onclick="opengraph();"></a></div>
        <?php endif; ?>

        <div
            style="position: absolute; width: 49px; height: 50px; background: url(star.png) no-repeat; background-size: 50px; margin-top: 100px; margin-left: 15px;">
            <b style="cursor: pointer; display: block; padding: 18px 0; text-align: center;" class="stars"
               onclick="$.fancybox.open({content:$('#payment').html(),topRatio:0,margin:[100, 20, 20, 20],autoHeight:true,height:'auto'});"
               title="<?php echo $stars; ?>"><?php echo $stars_; ?></b>
        </div>

        <div style="float: left; margin-top: 10px; width: 280px;">
            <img src="" align="left" class="uphoto shadowing" width="70" height="70"
                 style="margin-right: 10px;margin-left: 0;margin-top: 0;"/>
        ...
        <div style="margin: 0 auto; clear: both; margin-top: 10px; overflow: hidden;"><h3>Интерпретации аспектов на
                <b><span class="attention cntdate" style="font-size: 14px;"><?php echo $cntdate; ?></span></b></h3>
        </div>
        <div id="goro" style="clear: both; overflow: hidden;">
            <?php if ($gsk): ?>
                <div class="mygoro shadowing"
                     style="overflow: hidden; clear: both;margin-top: 10px; float:left; border-radius: 10px; padding: 10px; background: white;">
                    <?php require_once('makegraph.php'); ?>
                </div>
            <?php else: ?>
                <h3 class="aligncenter" style="padding-top: 20px;">Предназначено для использования в социальных
                    сетях.</h3>
            <?php endif ?>
        </div>
        <?php if (!$gsk && !$fid): ?>
            <p></p>
            <h3>Смотреть гороскопы друзей</h3>
            <div id="friends" style="overflow: hidden;">
                ...
                <div class="pp" style="display: none;">старт/пауза</div>
            </div>
        <?php endif; ?>
    </div>
    <p style="font-size: 12px; color: #999; text-align: center; padding-bottom: 0;margin-bottom: 0;">(с) 2014-2015
        Андрей Перье и
        <a href="http://best-horoscope.ru" target="_blank" style="font-size: 12px; text-decoration: none; color: #777;">Интернет-группа
            "Астроном и я"</a><br>
        Интерпретации разработаны с помощью <a href="http://astrohit.ru"
                                               style="font-size: 12px; text-decoration: none; color: #777;">команды
            профессиональных астрологов</a>
    </p>
    <div id="payment" style="display: none;">
        ...
        <p>
            Для просмотра интерпретаций аспектов необходимы <b>Звезды</b>. Каждый день, когда вы заходите в приложение
            <b>Астрологический календарь</b> вам начисляются три звезды.<br>
            Таким образом, у вас будет возможность просматривать интерпретации минимум трех аспектов ежедневно. Если по
            какой-либо причине вам будет не хватать ежедневных звезд, то вы всегда сможете получить их в обмен на голоса
            прямо здесь:
        </p>
        <center>
        ...
        <p>И, конечно, же в качестве приятного бонуса, вы получите 10 звезд за каждого вашего друга, получившего от вас
            приглашение и воспользовавшись приложением <b>Астрологический календарь</b>. Для приглашения друзей
            воспользуйтесь ссылкой <a href="javascript:void(0);" onclick="VK.callMethod(&quot;showInviteBox&quot;);">Пригласить
                друзей</a> на главной странице приложения.</p>
