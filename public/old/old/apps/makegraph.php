...
//	$check = "SELECT * FROM _ephemerides WHERE edate = '$edate' OR edate = '$cdate'";
    ...
    $descr = Array(
        '<p>
            <img src="sun.png" class="imgstar" align="left"/>
            <b>Солнце</b> проходит зодиакальный круг за 1 год (точнее за 365,4 суток). Центр нашей Солнечной системы, оно дает свет и тепло всем планетам. Вокруг него обращаются планеты и их спутники, карликовые планеты и их спутники, астероиды, кометы, метеориты и космическая пыль.
        </p>',
        '',
        '<p>
            <img src="mercury.png" class="imgstar" align="left"/>
            <b>Меркурий</b> – это самая маленькая планета Солнечной системы, которая находится ближе других к Солнцу. У Меркурия нет спутников. Поверхность планеты покрыта кратерами, возникшими около 3,5 млрд. лет назад вследствие массированных бомбардировок метеоритами.
        </p>',
        '<p>
            <img src="venus.png" class="imgstar" align="left"/>
            <b>Венера</b> имеет период обращения по Зодиаку около одного года. Это вторая по близости к Солнцу планета Солнечной системы. Своими размерами Венера лишь немного уступает Земле, поэтому ее иногда называют «сестрой Земли». Спутников не имеет.
        </p>',
        '<p>
            <img src="mars.png" class="imgstar" align="left"/>
            <b>Марс</b> имеет период обращения в Зодиаке в среднем 1 год и 10 месяцев. Это первая внешняя по отношению к Земле планета и седьмая по размеру планета в Солнечной системе. «Красная планета», как его еще называют благодаря наличию большого количества оксида железа в грунте. У Марса есть два спутника: Деймос и Фобос.
        </p>',
        '<p>
            <img src="jupiter.png" class="imgstar" align="left"/>
            <b>Юпитер</b> – самая большая планета Солнечной системы. Она в 318 раз тяжелее Земли, и почти в 2,5 раза массивнее, чем все планеты нашей системы вместе взятые. По своему составу Юпитер напоминает Солнце – он состоит преимущественно из гелия и водорода – и излучает огромное количество тепла, равное 4*1017 Вт.
        </p>',
        '<p>
            <img src="saturn.png" class="imgstar" align="left"/>
            <b>Сатурн</b> тоже относится к планетам-гигантам, хотя он и меньше Юпитера. Сатурн – вторая по размеру планета Солнечной системы. Визитная карточка Сатурна – это, конечно же, его система колец, которая состоит в основном из ледяных частиц разного размера (от десятых долей миллиметра до нескольких метров)
        </p>',
        '<p>
            <img src="uranium.png" class="imgstar" align="left"/>
            <b>Уран</b> открыт в 1781 году. Полный оборот вокруг Солнца он делает за 84 года, находясь примерно по 7 лет в каждом знаке Зодиака. Уран – это первая планета, обнаруженная с помощью телескопа, а также единственная планета в Солнечной системе, которая оборачивается вокруг Солнца, «лежа на боку», да еще и в обратную сторону.
        </p>',
        '<p>
            <img src="neptune.png" class="imgstar" align="left"/>
            <b>Нептун</b> - восьмая и самая дальняя планета Солнечной системы. Нептун также является четвёртой по диаметру и третьей по массе планетой. Масса Нептуна в 17,2 раза, а диаметр экватора в 3,9 раза больше таковых у Земли[9]. Планета была названа в честь римского бога морей
        </p>',
        '<p>
            <img src="pluto.png" class="imgstar" align="left"/>
            <b>Плутон</b> - одна из крупнейших известных (наряду с Эридой) карликовых планет Солнечной системы, транснептуновый объект (ТНО) и десятое по массе (без учёта спутников) небесное тело, обращающееся вокруг Солнца[3][4][5]. Первоначально Плутон классифицировался как классическая планета, однако сейчас он считается карликовой планетой. Из-за эксцентричности орбиты Плутон то приближается к Солнцу на расстояние 29,6 а. е. (4,4 млрд км), оказываясь к нему ближе Нептуна, то удаляется на 49,3 а. е. (7,4 млрд км).
        </p>',
    ...
    while($row = mysql_fetch_object($aspres)){
        ...
        $pl2 = $row->id_planet2;
        $degree = $row->degrees;
        if($pl1-1==$star || $pl2-1==$star){
            $myval = $pl1.$degree.$pl2;
            if(!in_array($myval, $myvals)) continue;
            $symbol[$pl1.$degree.$pl2] = Array($symbols[$pl1-1].' '.$symorb[$degree].' '.$symbols[$pl2-1]);
            $aspect[$pl1.$degree.$pl2] = Array(
                //Заголовок аспекта
                $planet1[$pl1-1].' '.$planet2[$pl1-1].' '.$inorb[$degree].' с '.$planet3[$pl2-1].' - <span style="font-family: fantasy; font-size: 16px;">'.$symbols[$pl1-1].' '.$symorb[$degree].' '.$symbols[$pl2-1].'</span>',
                //Текстовая часть аспекта
                //$row->aspects,
                "Угол ".$degree."&deg; с отклонением &plusmn;3&deg;",
    ...
    foreach($aspects as $mydate => $dateasp){
        ...
        for($i=0; $i<sizeOf($dateasp); $i++){
            ...
            if($cdate == $mydate){
                ...
                $asp .= '<p class="rating"><b>'.$aspect[$dateasp[$i]][0].'</b><br>'.$aspect[$dateasp[$i]][1].'</p>';
            }
    ...
    if($gsk && $stars<=0 && !$viewed){
        ...
        if (!$asp){
            echo "<b>В этот день планеты не выстроились в значимые аспекты относительно планеты ".ucwords($planet2[$star])."</b>";
        }else{
            //echo "<span>Вы можете разместить любой из аспектов у себя на стене, кликнув в любом месте в абзаце с аспектом.</span>";
            
            echo "<div style='background: white; border-radius: 10px; padding: 5px 10px;' class='shadowing'>".$asp."</div>";
        }
        echo "<h3>Полезная информация</h3>";
        echo $descr[$star];
    }
    ...
    for($i=0; $i<=30; $i++){
        ...
        $newdate = date("Ymd", strtotime("$udt + $i days"));
        $newdate_ = date("d.m.Y", strtotime($newdate));
        $tmplink = 'gsk='.md5($id_gorogroup.$uid.$newdate)."&uid=$uid&udt=$newdate&ubd=$ubd";
