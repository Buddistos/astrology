    ...
    if($gsk){
        if($uid){
            ...
            if($exists){
                ...
                $query = "SELECT * FROM _vkviews WHERE vkgsk = '$gsk';";
                $result = mysql_query($query);
                $exists = mysql_num_rows($result);
                if(!$exists && $stars>0){
                    $stars--;
                    $query = "INSERT INTO _vkviews (vkgsk) VALUES ('$gsk');";
                    $result1 = mysql_query($query);
                    $query = "UPDATE _vkapps SET vkstars = $stars WHERE vkuser = $uid;";
            ...
            VK.init(function() { 
             ...
             // Your code here 
                VK.api("getProfiles", {uids:window.viewer_id, fields:"bdate, sex, photo_medium"}, function(data) { 
                    ...
                    gsk = "<?php echo $gsk; ?>";
                    if(!gsk){
                        $("#goro").load("body.php",{uid: uid, bdate: bdate, uname: uname, referrer: referrer}, function(){
                            VK.api("friends.getAppUsers", {uid: uid}, function(data) { 
                                // Действия с полученными данными 
                                var fr = data.response;
                                if(fr.length){
                                    VK.api("users.get", {user_ids: fr, fields: "photo_medium"}, function(data) { 
                                        fr = data.response.length;
                                        if(fr){
                                            html = "";
                                            var mfr = 5-fr<1 ? 1 : 5-fr;
                                            for(var i=0; i < mfr; i++){
                                            ...
                                            for(var i=0; i < fr; i++){
                                                html += "<li style='float: left;'><a href='javascript:void(0);' onclick='$(\"#showcal\").click();'><img class='shadowing' src='"+data.response[i].photo_medium+"' width='50' height='50'></a></li>";
                                            }
                            ...
                            VK.callMethod('resizeWindow', 600, $('body').height()+30);
                        });
                    }else{
                        $(".uname").text(uname);
                        //$(".bdate").text(bdate);
...
<body>
    <div class="container">
        ...
        <div class="block">
            <?php if(!$guest && $gsk): ?>
                <div id="goback"><a href="/vkapps/"><img src="/images/arrow-left.png" width="40"/></a></div>
            <?php endif; ?>
