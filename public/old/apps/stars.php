...
<?php if($uid): ?>
    <?php
        foreach($planets as $i => $plnam){
            if($i==1)continue;
            echo "<div class='star' id='$plnam'><a style='display:block;text-align:center;' href='?gsk=".$cgsk[1]."&uid=".$uid."&udt=".$udt."&ubd=".$ubd."&star=".$i."'><img width='100' src='$planets[$i].png'/><br><b>$planet2[$i]</b></a></div>";
        }
/*
        for($i=1; $i<=7; $i++){
            $sgn = $goroname[$i];
